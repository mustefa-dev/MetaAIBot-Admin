  ...
  <UiCard>
    ...
    <div v-if="loading" class="space-y-4">
      <div v-for="i in 5" :key="i" class="flex items-center gap-3">
        <UiSkeleton class="w-10 h-10 rounded-full" />
        <div class="flex-1">
    ...
    <div v-else class="space-y-4">
      <NuxtLink
        ...
        class="flex items-start gap-3 p-2 -mx-2 rounded-lg hover:bg-gray-50 dark:hover:bg-gray-800 transition-colors"
      >
        <!-- Platform Icon -->
        <div
          class="w-10 h-10 rounded-full flex items-center justify-center shrink-0"
          :class="getPlatformBgClass(conv.platform)"
        >
          <component :is="getPlatformIcon(conv.platform)" class="w-5 h-5 text-white" />
        </div>
        ...
        <div class="flex-1 min-w-0">
          <div class="flex items-center justify-between gap-2">
            <span class="font-medium text-gray-900 dark:text-white truncate">
              {{ conv.customerName || t('customers.name') }}
            </span>
            <span class="text-xs text-gray-500 shrink-0">
              {{ format.relativeTime(conv.lastMessageAt) }}
            </span>
          </div>
          ...
          <div class="flex items-center gap-2 mt-1">
            <UiBadge :variant="getStatusVariant(conv.status)" size="sm">
              {{ t(`conversations.statuses.${conv.status}`) }}
            </UiBadge>
            <span v-if="conv.unreadCount" class="text-xs bg-red-500 text-white px-1.5 py-0.5 rounded-full">
              {{ conv.unreadCount }}
            </span>
          </div>
        </div>
